...
function usage() {
    prog=`basename $0`
    echo "$prog [-ndf0TI] [-U user ] -s schema [-m mintable] pattern"
    echo "        -d    debug"
...
s_dsn="--cluster 8.3/main -U $PGUSER source_db"
d_dsn="--cluster 8.4/main -U $PGUSER target_db"
...
do
	...
	(cd $TMPDIR && \
	 awk '{print "DROP INDEX " $3 ";"}' < $t.idx.sql > $t.idx.drop.sql)
